...
def lstm_model(embed):
    ...
    #embedding = tf.Variable(tf.random_uniform([VOCAB_SIZE, EMBEDDING_DIM], -1.0, 1.0), trainable=True, name="W_text")
   ...
   # lstm_cell_3 = tf.nn.rnn_cell.BasicLSTMCell(UNITS_NUM)
    ...
    # W和b，就是接在hidden_layer之后的全连接层；它下接hidden_layer，上接pred_class
    # W = tf.Variable(tf.truncated_normal( [PROJECTION_NUM, CLASS_NUM],mean=0.0,stddev=0.001), dtype=tf.float32)  # 生成 mean=0，std=1随机值   维度：128 X 1
    W = tf.get_variable("W", shape=[UNITS_NUM, CLASS_NUM] ,initializer=tf.contrib.layers.xavier_initializer()) 

    b = tf.Variable(tf.constant(0.1, shape=[CLASS_NUM]), name="b", dtype=tf.float32)   # 维度：2
    ...
    logits = tf.nn.xw_plus_b(all_outputs[-1], W, b) # outputs[-1] 为最后一个时刻的输出: [None,128]. 因此 results = None X 128 X 128 X 2 + 2 = None X 2 + 2 = None X 2
    ...
    #correct_prediction = tf.equal(tf.to_int64(predictions > 0.5),tf.argmax(target,1))
    #accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    correct_predictions = tf.equal(predictions, tf.argmax(target, axis=1))
    ...
    return BANTCH_SIZE, loss, accuracy, input_data, target, logits, all_outputs, W, b, embedding, predictions, dropout_keep_prob
